...
import com.serinus.loto.services.stats.LototurfStatsService
...
class LototurfStats @Inject() (lototurfStatsService: LototurfStatsService) {
  ...
  def computeLeastFrequentCombination(): Future[Either[StatsError, FreqMap]] = {
    computeFrequencies() map {
      case Right(freqs) => Right(freqs.toList.drop(25))
      case err @ Left(_) => err
    } recover {
      case err => Left(s"${err.getMessage}")
    }
  ...
  def computeFrequencies(): Future[Either[StatsError, FreqMap]] = {
    lototurfStatsService.findAllMainResults() map { resList =>
